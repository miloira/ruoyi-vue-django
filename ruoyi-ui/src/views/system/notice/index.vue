    ...
    <el-table v-loading="loading" :data="noticeList" @selection-change="handleSelectionChange">
      ...
      <el-table-column label="公告类型" align="center" prop="noticeType" width="100">
        <template slot-scope="scope">
          <dict-tag :options="dict.type.sys_notice_type" :value="scope.row.notice_type"/>
        </template>
      </el-table-column>
        ...
        <el-row>
          ...
          <el-col :span="24">
            <el-form-item label="内容">
              <editor v-model="form.notice_content" :min-height="192"/>
            </el-form-item>
          </el-col>
        </el-row>
      </el-form>
...
export default {
  ...
  methods: {
    ...
    handleUpdate(row) {
      this.reset();
      const notice_id = row.notice_id || this.ids
    ...
    /** 删除按钮操作 */
    handleDelete(row) {
      const notice_ids = row.notice_id || this.ids
      this.$modal.confirm('是否确认删除公告编号为"' + notice_ids + '"的数据项？').then(function() {
