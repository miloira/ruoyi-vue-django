    ...
    <el-dialog :title="title" :visible.sync="openDataScope" width="500px" append-to-body>
      <el-form :model="form" label-width="80px">
        ...
        <el-form-item label="数据权限" v-show="form.data_scope == 2">
          ...
          <el-checkbox v-model="form.dept_check_strictly" @change="handleCheckedTreeConnect($event, 'dept')">父子联动</el-checkbox>
          <el-tree
            class="tree-border"
            :data="deptOptions"
            show-checkbox
            ...
            ref="dept"
            node-key="id"
            :check-strictly="!form.dept_check_strictly"
            empty-text="加载中，请稍候"
            :props="defaultProps"
          ></el-tree>
        </el-form-item>
...
export default {
  ...
  methods: {
    ...
    getRoleMenuTreeselect(role_id) {
      return roleMenuTreeselect(role_id).then(response => {
        this.menuOptions = response.menus;
        return response;
      });
    },
    /** 根据角色ID查询部门树结构 */
    getRoleDeptTreeselect(role_id) {
      return roleDeptTreeselect(role_id).then(response => {
    ...
    handleUpdate(row) {
      ...
      const role_menu = this.getRoleMenuTreeselect(role_id);
      getRole(role_id).then(response => {
        this.form = response.data;
        this.open = true;
        this.$nextTick(() => {
          role_menu.then(res => {
            let checked_keys = res.checked_keys
            checked_keys.forEach((v) => {
                this.$nextTick(()=>{
      ...
      getRole(row.role_id).then(response => {
        ...
        this.$nextTick(() => {
          roleDeptTreeselect.then(res => {
            this.$refs.dept.setCheckedKeys(res.checked_keys);
          });
        });
        this.title = "分配数据权限";
      });
    ...
    handleAuthUser: function(row) {
      ...
      this.$router.push("/system/role-auth/user/" + role_id);
    },
    /** 提交按钮 */
    submitForm: function() {
    ...
    handleDelete(row) {
      ...
      this.$modal.confirm('是否确认删除角色编号为"' + role_ids + '"的数据项？').then(function() {
