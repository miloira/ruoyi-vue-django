    ...
    <el-dialog :title="title" :visible.sync="open" width="500px" append-to-body>
      <el-form ref="form" :model="form" :rules="rules" label-width="80px">
        <el-form-item label="字典类型" prop="dict_type">
          <el-input :value="to_dict_type(form.dict_type)" :disabled="true" />
        </el-form-item>
...
export default {
  ...
  created() {
    const dict_id = this.$route.params && this.$route.params.dictId;
    this.dict_id = this.$route.params && this.$route.params.dictId;
    this.getType(dict_id);
    this.getTypeList();
  },
  methods: {
    to_dict_type(dict_type_id) {
      if (Number.isInteger(dict_type_id)) {
        for (let dict_type of this.typeOptions) {
          if (dict_type.dict_id === dict_type_id) {
    ...
    /** 删除按钮操作 */
    handleDelete(row) {
      const dict_codes = row.dict_code || this.ids;
      this.$modal.confirm('是否确认删除字典编码为"' + dict_codes + '"的数据项？').then(function() {
        return delData(dict_codes);
      }).then(() => {
        this.getList();
